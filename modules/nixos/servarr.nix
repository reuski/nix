{ ... }:
{
  flake.modules.nixos.servarr =
    ...
    let
      ...
      setupScript = name: app: pkgs.writeShellScript "${name}-initial-auth" ''
        set -euo pipefail
      ...
      cleanupScript = name: app: pkgs.writeShellScript "${name}-cleanup-initial-auth" ''
        set -euo pipefail
    ...
    in
    {
      ...
      config = mkIf cfg.enable {
        services = mkMerge (
          ...
          ++ mapAttrsToList (name: app: mkIf app.enable {
            ${name}.settings = {
        ...
        systemd.services = mapAttrs (name: app: mkIf app.enable {
